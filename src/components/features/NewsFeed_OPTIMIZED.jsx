...
const NewsCard = React.memo(({ item, searchQuery, onQuizClick }) => {
  // 1. Hooks for 3D hover effect (disabled on mobile for performance)
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);

  // 2. Transform mouse position into 3D rotation
  const rotateX = useTransform(mouseY, [-150, 150], IS_MOBILE ? [0, 0] : [10, -10]);
  const rotateY = useTransform(mouseX, [-150, 150], IS_MOBILE ? [0, 0] : [-10, 10]);
  ...
  const handleMouseMove = useCallback(({ currentTarget, clientX, clientY }) => {
    if (IS_MOBILE) return; // Skip on mobile for better performance
  ...
  return (
    ...
    >
      ...
      {/* 5. The mouse-following glow effect (only on desktop) */}
      {!IS_MOBILE && (
        <motion.div
          className="card-glow"
          style={{
...
export function NewsFeed({ searchQuery, date, yearRange, activeFilter }) {
  ...
  const fetchDateData = useCallback(async (month, day) => {
    ...
    if (searchQuery && searchQuery.trim()) {
      ...
      const timeoutId = setTimeout(async () => {
        ...
        try {
          ...
          const response = await fetch(searchUrl, { signal: abortControllerRef.current.signal });
  ...
  }, [date, searchQuery, fetchDateData]);
  ...
  const feedVariants = useMemo(() => ({
    ...
    visible: {
      ...
      transition: {
        ...
        staggerChildren: IS_MOBILE ? 0.02 : 0.05
      }
    }
  }), []);
  ...
  return (
    <>
      <motion.main
        ...
        initial="hidden"
        animate="visible"
        layout={!IS_MOBILE} // Disable layout animations on mobile
      >
        {loading ? (
